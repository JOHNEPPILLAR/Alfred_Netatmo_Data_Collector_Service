...
async function _sensors(req, res, next) {
  this.logger.trace(`${this._traceStack()} - Display Netatmo data API called`);
  ...
  try {
    ...
    this.logger.debug(`${this._traceStack()} - Get sensor values`);
    ...
    if (results.rowCount === 0) {
      this.logger.debug(`${this._traceStack()} - No data to return`);
